...
console.log = (...args) => {
    ...
    if (msg.includes('fake worker') || msg.includes('Unsupported') || msg.includes('NOT valid form')) return;
    originalLog(...args);
};
...
router.post('/api/student/analyze-resume', requireAuth,  async (req, res) => {
    try {
...
const prompt = `You are a senior technical recruiter. Analyze this resume and give a quick recruiter-friendly summary.
...
Rules:
- Section feedback must be MAX 1 sentence each, direct and actionable
- Suggestions must be exactly 3, professional, specific and impactful
- matchedKeywords: only strong technical matches
- missingKeywords: only relevant missing skills for this role
- candidateSummary: 2-3 lines MAX, written for a busy recruiter — cover key skills, standout projects, and overall fit. Be direct and specific, no fluff.

Return ONLY valid JSON, no markdown, no extra text:
{
  "score": <number 0-100>,
  "summary": "<one crisp sentence: overall verdict>",
  "candidateSummary": "<2-3 line recruiter snapshot: key skills, best project, overall fit>",
